...
export default function Patientimage() {
  ...
  const currentdatas = itemData.slice(indexoffirstdata, indexoflastdata);
  const pagenumbers = [];
  for (var i = 1; i <= Math.ceil(itemData.length / datasperpage); i++) {
    pagenumbers.push(i);
  }
          ...
          return (
            <Box sx={{ margin: { lg: "5px" } }}>
              <img
                ...
                srcSet={`${item.img}?w=100&fit=crop&auto=format`}
                alt={item.title}
                loading="lazy"
              />
